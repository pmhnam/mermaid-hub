...
### Product browser test

`pnpm test:e2e:product` runs the workspace product flow in Chromium and starts both the Vite
frontend and the built `../diagram-api` application. PostgreSQL must already be available and migrated at
`localhost:5432` with the `diagram:diagram` credentials; the test does not create, migrate, reset,
or destroy the database. The Vite development server proxies `/api` and `/ws` to
`MERMAID_API_PROXY_URL`, which defaults to `http://localhost:3001`.
